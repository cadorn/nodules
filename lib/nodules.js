...
var dump = function() {},
    ...
    sync = system.env.NODULES_SYNC || false,
    debug = system.env.NODULES_DEBUG || false;
...
filePathMappingsJson = filePathMappingsJson || system.env.NODULES_PATH_MAPPINGS || false;
		...
		if(mappings){
			mappingsArray = mappingsArray.concat(Object.keys(mappings).map(function(key){
				...
				if(typeof to == "string"){
                    ...
                    // always match paths
                    key += '/';
                    to = to.location.substring(0, to.location.length-1) + "!/";
				}
				return {
					from: key,
					exact: to.match(/\.\w+$/),
